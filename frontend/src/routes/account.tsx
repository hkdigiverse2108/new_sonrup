...
function ProfilePanel() {
  const { user, updateUser, orders } = useAuth();
    ...
    <div className="grid gap-6 md:grid-cols-2">
      <div className="surface-card p-7 md:col-span-2">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            updateUser({ name: name.trim() || user!.name, email: email.trim() || user!.email, phone });
            setEditing(false);
            toast.success("Profile updated");
          }}
          className="mt-7 grid gap-5 sm:grid-cols-3"
        >
          <div className="rounded-2xl bg-muted/60 px-5 py-4 focus-within:ring-2 focus-within:ring-primary transition-all">
            <label className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-muted-foreground block">Name</label>
            {editing ? (
              <input
                autoFocus
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="mt-1.5 w-full bg-transparent text-sm font-bold outline-none placeholder:text-muted-foreground"
              />
            ) : (
              <p onClick={() => setEditing(true)} className="mt-1.5 truncate text-sm font-bold cursor-text hover:text-primary transition-colors">{user?.name}</p>
            )}
          </div>
          
          <div className="rounded-2xl bg-muted/60 px-5 py-4 focus-within:ring-2 focus-within:ring-primary transition-all">
            <label className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-muted-foreground block">Email</label>
            {editing ? (
              <input
                type="email"
                value={email}
              ...
              <p onClick={() => setEditing(true)} className="mt-1.5 truncate text-sm font-bold cursor-text hover:text-primary transition-colors">{user?.email}</p>
            )}
          </div>
          
          <div className="rounded-2xl bg-muted/60 px-5 py-4 focus-within:ring-2 focus-within:ring-primary transition-all">
            <label className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-muted-foreground block">Phone</label>
            {editing ? (
              <input
                value={phone}
              ...
              <p onClick={() => setEditing(true)} className="mt-1.5 truncate text-sm font-bold cursor-text hover:text-primary transition-colors">{user?.phone || "Not added yet"}</p>
            )}
          </div>
      ...
      <div className="surface-card flex items-center gap-4 p-7">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Lifetime value {inr(orders.reduce((s: number, o: any) => s + o.total, 0))}
          </p>
        </div>
      </div>
    </div>
  ...
  if (orders.length === 0) {
    return (
      <div className="surface-card p-12 text-center text-muted-foreground">
        <Package className="mx-auto h-8 w-8 mb-4 opacity-50" />
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Reveal key={a.id} delay={i * 70}>
            <div className="surface-card relative h-full p-6">
              <div className="flex items-center gap-2">
                ...
                {a.isDefault && (
                  <span className="rounded-full bg-secondary/10 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.16em] text-secondary">
                    Default
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 flex items-center gap-3">
                <button
                  onClick={() => {
                    setForm({ ...a, isDefault: a.isDefault ?? false });
                    setEditingId(a.id);
                    setOpen(true);
                    // scroll to form
                    setTimeout(() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }), 50);
                  }}
                  className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-foreground transition-colors hover:text-primary"
                >
                  Edit
                </button>
                {!a.isDefault && (
                  <button
                    ...
                    className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-primary transition-colors hover:text-primary/70"
                  >
                    Make Default
                  </button>
                )}
        ...
        <button
          onClick={() => {
            setForm({ label: "Home", name: "", line1: "", city: "", state: "", pincode: "", phone: "", isDefault: false });
            setEditingId(null);
            setOpen((o) => !o);
          }}
      ...
      {open && (
        ...
        >
          ...
          <div className="sm:col-span-2 mt-2 flex gap-3">
            ...
            <BrandButton type="button" variant="outline" onClick={() => {
              setOpen(false);
              setEditingId(null);
            }}>
              Cancel
